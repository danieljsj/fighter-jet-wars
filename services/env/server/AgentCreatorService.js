...
// NOTE: these two might should be common...
	...
	////// NOPE: DO THIS FROM THE IO: MOVE IT TO THERE: /////////////////////////////////////

 	playerAdditionRef = ref.child('playerCrud').push();

 	playerAdditionRef.set({
 		id: playerAdditionRef.key,
 		type: 'join'
 	});



 	// do the same with units probs.
...
function listenToFbUserAdds(){
	...
	ref.child('users').on('child_added', function(ss, prevChildId){
		...
		addPlayerAndDefaultUnits({user:user}); ////////hmm. looks like the player contains the user object completely. Which I guess makes sense. User is outside the game, player is the game object. it's 1:1 or 1:0, so user, within the scope of a game, really is just a property of a player.
	});
